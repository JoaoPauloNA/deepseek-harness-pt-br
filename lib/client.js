...
const dictionaries = {
  conversation: {
    'view.chat': 'Conversa', 'placeholder.default': 'Envie uma mensagem ao agente',
    'placeholder.hero': 'Descreva o que você quer criar', 'placeholder.workspace': 'Escolha um espaço de trabalho para começar',
    'placeholder.unavailable': 'Sessão indisponível', 'input.commands': 'Comandos', 'input.stop': 'Parar geração',
    'input.send': 'Enviar mensagem', 'hero.preview': 'Prévia', 'hero.chooseWorkspace': 'Escolher espaço de trabalho',
    'details.title': 'Detalhes', 'details.close': 'Fechar detalhes', 'details.input': 'Entrada', 'details.output': 'Saída',
    'details.running': 'Em execução…', 'todo.title': 'Tarefas', 'chat.loadingHistory': 'Carregando histórico…',
    'chat.loadOlder': 'Carregar anteriores', 'chat.toBottom': 'Ir para o fim', 'message.stopped': 'Interrompido',
    'message.turnError': 'A execução desta rodada falhou', 'command.running': 'Em execução…',
    'command.failed': 'Comando falhou', 'command.done': 'Concluído', 'approval.waiting': 'Aguardando aprovação',
    'approval.reject': 'Recusar', 'approval.allowOnce': 'Permitir uma vez',
  },
  workspace: {
    'session.new': 'Nova sessão', 'section.workspaces': 'Espaços de trabalho', 'section.sessions': 'Sessões',
    'workspace.add': 'Adicionar espaço de trabalho', 'search.sessions.aria': 'Pesquisar sessões',
    'search.placeholder': 'Pesquisar sessões…', 'search.clear': 'Limpar pesquisa', 'empty.none': 'Ainda não há sessões',
    'empty.noMatches': 'Nenhum resultado', 'rename': 'Renomear', 'delete.workspace': 'Excluir espaço de trabalho',
    'menu.fork': 'Criar ramificação da sessão', 'menu.archiveSession': 'Arquivar sessão', 'status.running': 'Em execução',
    'status.idle': 'Ocioso', 'status.waitingApproval': 'Aguardando aprovação', 'status.completed': 'Concluído',
  },
  goal: {
    'phase.active': 'Meta em andamento', 'phase.paused': 'Meta pausada', 'phase.blocked': 'Meta bloqueada',
    'action.save': 'Salvar meta', 'action.cancel': 'Cancelar edição', 'action.pause': 'Pausar meta',
    'action.resume': 'Retomar meta', 'action.edit': 'Editar meta', 'action.clear': 'Limpar meta',
  },
}
 ...
 * @deepseek-ai/dsh-client-locale ships a closed locale set (LOCALE_IDS and
 * the Settings language row's LOCALES are hardcoded to ['zh', 'en'];
 * setLocale() throws for any other id). Dictionary registration alone (the
 * block below) lands translations in the lookup table but never makes
 * "Português (Brasil)" appear as an option. There is currently no supported
 * extension point for a third-party package to add a new locale id. Until
 * the harness ships one, this function mutates the running LocaleRuntime
 * instance directly (its `snapshot`/`publish`/`setLocale` are `private` only
 * at the TypeScript layer — plain instance members at runtime).
 *
 * What the patch does NOT do: persist the pt-BR preference through the Host
 * settings service, because LocaleSettingsSchema (also zh/en-only) would
 * reject that write. The selection is remembered in localStorage and
 * reapplied on boot instead. Caveat: if the Host settings section for
 * `locale` changes for any other reason while pt-BR is active (e.g. a sync
 * from another tab), the runtime's own adopt() falls back to the
 * Host-stored zh/en preference; reselecting the language recovers it.
 */
function apply(ctx) {
 ...
 */
function patchLocaleRuntime(runtime) {
